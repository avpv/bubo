...
> **Last ingest:** 2026-07-16 (rev: capture now parses a trailing duration — `QuickCaptureView` previews `BacklogTitleParser.parse` live («Task · 30 min», tilde for verb-table guesses), and the menu-bar listener `handleCapturedBacklogTask` commits through `handleQuickAddTask`, the same path as the in-popover Quick Add; the ⇧↩ details path and the `QuickCaptureBridge` drain also prefill the parsed duration. Prior rev: line refs in `AppDelegate+QuickCapture.swift` bumped +1 — `installQuickCaptureHotkey` `:37`, local monitor `:47`, global `:62`, `spaceKeyCode` `:31`, `toggleQuickCapture` `:73`, RegisterEventHotKey comment `:27`)
...
3. The user types a task. A trailing duration («30m» / «1h30m») is read by `BacklogTitleParser` — the same parser as the in-popover Quick Add and the backlog composer — and previewed live under the field («Task · 30 min»; a verb-table guess wears a `~` tilde). `QuickCaptureView` writes the raw buffer into `QuickCaptureBridge` (`Presentation/State/QuickCaptureBridge.swift`).
4. `⇧↩` opens the full `NewTaskView` pre-filled from the bridge buffer for detailed editing (deadline, recurrence, project, subtasks); the trailing duration is parsed out into the form's duration field (`MenuBarView+Lifecycle.swift:handleCapturedBacklogTaskWithDetails`).
5. Plain `↩` commits via `MenuBarView+Lifecycle.swift:handleCapturedBacklogTask`, which re-runs the same parse and delegates to `handleQuickAddTask` (`MenuBarView+MainContent.swift`) — the single commit path shared with the in-popover Quick Add — persisting through `BacklogService.addTask(...)` with an undo toast.
