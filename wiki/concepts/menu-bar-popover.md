...
Below the header, today's own `DaySectionHeader` (`Components/Event/DaySectionView.swift`) renders an **empty** summary line — its old «9–22 · 0 events» duplicated the popover header one band above (`summaryString` returns `""` when `isToday`). Other days still show `count + meta`. Today's title also collapses to the single accent word «Today» (no eyebrow); tomorrow keeps eyebrow «Tomorrow» + weekday-and-date title; further-out days show eyebrow = weekday, title = date only (`DaySectionView.swift:` `dayTitle`/`isToday` branch).
