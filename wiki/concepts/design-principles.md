...
- `Presentation/Views/DesignSystem/DesignSystem.swift` — the `DS` namespace (sizes, spacing, fonts, animations) is the canonical source for tokens. Magic numbers in feature views are a smell.
- `Presentation/Views/Common/BuboSkin.swift` — the only path through which skin-themable mood properties reach views.
