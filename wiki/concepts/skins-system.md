...
> **Last ingest:** 2026-06-10 (rev: PR #559 — fixed fontDesign line reference :166→:227; added defaultSkin fallback chain and selectedSkinID default "classic" to Active skin section)
...
Font design (face family) is a per-skin property since PR #553: `SkinDefinition.fontDesign` (`SkinDefinition.swift:227`) accepts `SkinFontDesign` (`.rounded`, `.default`, `.serif`, `.monospaced`). The default is `.rounded` (SF Rounded), so existing skins are bit-identical. PRINCIPLES §8's "SF Rounded is fixed" constraint no longer applies — the built-in Apple skin uses `.default` (SF Pro Text/Display).
...
`ReminderSettings.selectedSkinID` (`Domain/Reminders/ReminderSettings.swift:106`) stores the active skin id; the hardcoded default is `"classic"` (`:209`). The derived `selectedSkin: SkinDefinition` (`:117`) resolves via `SkinCatalog.skin(forID:)`. `SkinCatalog.defaultSkin` uses the fallback chain classic → system → apple → first-available (`SkinDefinition.swift:647`). `BuboSkin.swift` defines the `\.activeSkin` SwiftUI environment key (`:281`) so any view can read the current `SkinDefinition` via `@Environment(\.activeSkin)`.
