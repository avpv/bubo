...
`CPSATRepair` (in `GeneticAlgorithm/`, `struct CPSATAssignment` at `CPSATRepair.swift:74`) is a CDCL-lite constraint solver with Luby restarts and VSIDS-like activity bumping. Two roles:
