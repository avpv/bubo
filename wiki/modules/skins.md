...
> **Last ingest:** 2026-06-04 (rev: Flat skin added as new default built-in; shadow/opacity defaults tightened across `SkinDefinition`; display order updated to list Flat first; PR #557)
...
| `BuiltInSkins/` | — | 15 bundled `.json` files | `Flat` (default — 2026 redesign; accent #0071E3, `fontDesign: .default`, `buttonShape: .roundedRect`), `Apple`, `Arctic`, `Classic`, `Graphite`, `Lavender`, `Midnight`, `Ocean`, `RoseGold`, `Sage`, `Sierra`, `System`, `WinXPBlue`, `WinXPOlive`, `WinXPSilver` |
...
Typography family was historically fixed to SF Rounded; `fontDesign` is a per-skin choice since PR #553. The Apple and Flat built-in skins both use `.default` (SF Pro Text/Display); all other built-in skins retain `.rounded` (SF Rounded) and are bit-identical.
