...
| `Calendar/OptimizableEvent.swift` | 85 | `struct OptimizableEvent` (`:10`) | The optimizer's value type for "something to schedule" — title, duration, deadline, priority, energy, preferred hour range, pomodoroConfig, reservedTaskIds, dependsOn, etc. **Moved from `Bubo/Optimizer/Models/OptimizerModels.swift` to Domain on 2026-05-12** because `BacklogTask.toOptimizableEvent(...)` was already defined on a Domain type; keeping the conversion target in Optimizer/ produced a Domain↔Optimizer dependency cycle that blocked the multi-target split |
