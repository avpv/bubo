...
> **Last ingest:** 2026-07-16 (rev: core-audit correctness fixes, behaviour only — `BuboOptimizer.optimize(commitState:)` added so preview/dry-run/shadow runs no longer stamp `currentSchedule`/`lastResult`; `AppliedSnapshot` gained `removedEvents` + `previousTaskLinks` (`BacklogLinkSnapshot`) so undo restores deleted events and task links by task id; `FitnessEvaluator.evaluateDelta` flags constraint rejections (`EvaluationResult.isConstraintRejection`) instead of returning the parent's objective vector; rank selection off-by-one fixed; `run()` re-inserts `bestEver` into the returned population; `MutationBandit.record` takes an explicit per-island context; CP-SAT forward check is bidirectional and no-goods are value-scoped; `SlotDomain.build` enforces per-slot day-fit; `SlotRegistry.indexAtOrAfter` added for forward-only snaps; learning-state registry detects recycled addresses. Prior rev 2026-05-14: line citations resynced for `BuboOptimizer+Learning.swift`, `Scenarios/*`, `Training/*`, `Models/*`; section added for Anchors/)
