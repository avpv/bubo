...
|---|---|---:|---|
| `MenuBar/MenuBarView.swift` | `struct MenuBarView` (`:3`) | 1759 | Popover root. Orchestrates timeline, day navigation, day-rollover timer, initial sync status, toast state, scroll position. Nine logic clusters live in sibling extension files (`+AutoDefer`, `+RollForward`, `+Pomodoro`, `+Timeline`, `+BacklogDrop`, `+Strings`, `+EventActions`, `+Permissions`, `+Focus`). Nested types extracted to file scope: `MenuBarNavigation`, `MenuBarPaletteContext`, `MenuBarDayListItem`, `MenuBarTimelineDay`. Permission banners + settings button live in `Presentation/Views/Components/`; preference keys in `Presentation/Views/MenuBar/MenuBarPreferenceKeys.swift` |
...
|---|---:|---|
| `MenuBar/MenuBarView.swift` | 1759 | Body + view-builder helpers (`mainContent`, `eventList`, `eventRow`, `freeSlotRow`, `dayGroupSection`, `dayGroupHeader`, `collapsedEventsHeader`, `syncingState`, `nowMarkerRow`, `dayNavCluster`, `parallaxOffset`). Nine logic clusters in sibling files: `MenuBarView+AutoDefer.swift` (162, midnight rollover + EOD banner), `MenuBarView+RollForward.swift` (180, J-Recover + focus-slot fill + overdue reschedule), `MenuBarView+Pomodoro.swift` (206, convertEventToPomodoro + clone-as-draft + ripple-shift + slot Pomodoro), `MenuBarView+Timeline.swift` (163, `filteredEventsByDay` / `timelineEventsByDay` / `visibleEventCount` / `timelineDays` / `interleave` / `startOf` / `ghostForDay`), `MenuBarView+BacklogDrop.swift` (230, `handleTaskDrop` + `scheduleBacklogTask` + `scheduleSlotPickerBatch` — every path that places a backlog task onto the calendar), `MenuBarView+Strings.swift` (133, pure-compute strings: `headerTitle` / `headerSubtitle` / `dayHeaderMeta` / `emptyStateSubtitle` / `emptyFilteredStateMessage` / `usedColorTags`), `MenuBarView+EventActions.swift` (76, `resolveEdit` / `handleDelete` / `notifyScheduleChange` / `runQuickAction`), `MenuBarView+Permissions.swift` (71, `permissionBannerSpecs` / `refreshPermissionSnapshots` / `showSyncingState`), `MenuBarView+Focus.swift` (83, day-nav cluster state: `pendingTaskCount` / `isScrolledFromTop` / `focusedDayIndex` / `focusedDayIsToday` / `navigateToDay` / `todaysEventsForNowNext`). State surface (~30 `@State` fields), notable: `navigation: MenuBarNavigation` (state-machine enum), `dayRolloverTimer`, `everyMinuteTimer`, `initialSyncTimeoutFired` + `initialSyncDataArrived`, `extraDaysShown` capped at `extraDaysCap = 84`, `colorFilter` + `freeSlotFilter`, `backlogCoordinator`, `paletteContext`. `@State`/`@AppStorage` for `dayRolloverTimer`, `rollForwardDismissedDay`, `eodDismissedDay`, `toastState`, `navigation`, `nowTick`, `colorFilter`, `freeSlotFilter`, `backlogCoordinator`, `extraDaysShown`, `scrollPositionID`, `focusedDayDate`, `calendarHasAccess`, `remindersHasAccess`, `hasStartedSync`, `initialSyncDataArrived` is internal (was `private`) so siblings can mutate. |
...
Treat these as flagged for further decomposition only if needed — successive logic splits brought every "mega-file" below ~1000 lines except `MenuBarView.swift`, which dropped from 2400 → 1759 L on 2026-05-12 across six new extension files (`+Timeline`, `+BacklogDrop`, `+Strings`, `+EventActions`, `+Permissions`, `+Focus`). The deferred PRs in `BODY-SPLIT-PLAN.md` (mainContent extraction) are still the most leveraged remaining split for MenuBarView and Backlog.
