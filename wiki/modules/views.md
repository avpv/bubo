...
> **Last ingest:** 2026-06-11 (rev: MenuBarView 211→185 L; EventRowView 721→647 L; EventRowActions added; +EventRow now houses eventRowActions env struct; PR #568)
...
|---|---|---:|---|
| `MenuBar/MenuBarView.swift` | `struct MenuBarView` (`:4`) | 185 | Popover root — composition only. `var body` is ~50 L: background + `navigationDestination()` + command palette + toast + keyboard shortcuts, plus the modifier chain (includes `.environment(\.eventRowActions, eventRowActions)` at `:134` — one install covers every event row, PRINCIPLES §9). Fourteen logic/view clusters live in sibling extension files: prior nine (`+AutoDefer`, `+RollForward`, `+Pomodoro`, `+Timeline`, `+BacklogDrop`, `+Strings`, `+EventActions`, `+Permissions`, `+Focus`) plus five from the 2026-05-12 body decomposition (`+NavigationRoutes`, `+MainContent`, `+Lifecycle`, `+EventRow`, `+DayGroup`); `+EventRow` houses both `var eventRowActions: EventRowActions` (`:18`) and `eventRow()` builder (`:140`). The standalone focus-summary pill row and the separate command bar were removed in PR #553 — stats surface through day-section header subtitles; the optimizer entry point is the eyebrow+title block itself (Button → command palette). Nested types extracted to file scope: `MenuBarNavigation`, `MenuBarPaletteContext`, `MenuBarDayListItem`, `MenuBarTimelineDay`. Permission banners + settings button live in `Presentation/Views/Components/`; preference keys (`OptimizerBottomKey`) in `Presentation/Views/MenuBar/MenuBarPreferenceKeys.swift:13` |
...
### Event/task rows (6)
- `EventRowView` (`:4`) — single calendar-event row. Facts only: `event`, `reminderService`, `isLocked`, `isExcluded`, `energyAtStartHour`, `isFreshlyCreated`, `isHappeningNow`. All verbs read from `@Environment(\.eventRowActions)`.
- `EventRowActions` (`Components/Event/EventRowActions.swift:16`) — `struct EventRowActions`: 17-verb environment carrier (tap, edit, delete ×3, renameLocal, reschedule, completeTask, findBetterTime, splitTask, protectBlock, addPrep, addPrepQuick, convertToPomodoro, repeatLikeThis, toggleLock, toggleExclude). All closures optional; nil hides the affordance. Environment key at `:46`; `EnvironmentValues.eventRowActions` at `:53`.
- `BacklogTaskRow` (`:20`)
- `GhostEventRow` (`:14`)
...
| `Components/Event/EventRowView.swift` | 647 | Single-row component; verbs via `@Environment(\.eventRowActions)`. Three clusters split out 2026-05-12: `+Title.swift` (104, inline rename gate + commit/cancel), `+DragReschedule.swift` (134, vertical drag with minute snapping + preview badge), `+HoverActions.swift` (198, snooze/complete/disintegration-delete/reminder menu). Companion `EventRowActions.swift` (57) carries the 17-verb struct and its `EnvironmentKey`. |
