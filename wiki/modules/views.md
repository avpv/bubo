...
> **Last ingest:** 2026-07-18 (rev: PR #607 — backlog task rows restructured to a two-deck layout (title deck + up to `metaBudget = 4` metadata items, `+N` overflow tail); new `Components/Common/SnippetRowChrome.swift` (`RowStateStripe` + `snippetRowChrome()` modifier) unifies hover fill / keyboard-focus ring / drop-target highlight across `BacklogTaskRow` and `EventRowView`, replacing each row's hand-rolled overlays; `DS.Size.rowMinHeight` (40pt) replaces the split `eventRowMinHeight`/`backlogRowHeight`/`BacklogTaskRow.compactRowHeight` constants for event, backlog-task, free-slot, and ghost rows; component count 64 → 65). Prior: 2026-07-18 (rev: `Banner/SyncStaleBannerRow.swift` added — stalled-sync retry capsule for the popover status slot; component count 63 → 64). Prior: 2026-07-16 (rev: wiki-ingest for PR #593 — `FullScreenAlertView`'s action row is now Dismiss-leading/Join-trailing with a flat accent Join fill (gradient removed), and the bell icon skips `.symbolEffect` under Reduce Motion rather than slowing it. Prior rev: Settings fully migrated to native `Form(.grouped)` — Reminders/Appearance/Optimizer/Calendars/WorldClock/Assistant/AppleReminders tabs now match the GeneralTabView pattern (sections + headers/footers, per-account list sections); `SettingsPlatter.swift` deleted (zero users) and dead duplicate `AITabView.swift` deleted (AssistantTabView is the live UI); command-palette ↑/↓ moved onto the focused TextField; world-clock inline line lifted tertiary→secondary; live-wallpaper canvas bases are per-style in BackdropLegibility. Prior rev: design-review R1-R3 — `.tinted` button role (Join), flat solid primaries (no gradient/shine/glow), neutral countdowns, accent Now pill, day header on `skinBarBackground`, no surface-tint wash over wallpapers, `accentC20` skin-token fix. Prior rev: HIG compliance pass — `DS.Typography.row` voice added (`.subheadline`, the only sanctioned home); `PopoverHeader.backBindsEscape`; event context menu reorganized under an «Optimize» submenu; backlog «Move» submenu with hidden-not-dimmed directions; `StatusBanner` de-pilled; capacity ring drops accent from its color ramp; day-nav keyboard shortcuts. See root `HIG_COMPLIANCE.md`. Prior rev: backdrop legibility — new `Components/Background/BackdropLegibility.swift` forces content color scheme from wallpaper canvas luminance, adds a legibility scrim, and adapts the skin accent to the backdrop; `AppBackgroundLayer` no longer double-washes wallpapers with the skin gradient; `MenuBarView` applies `backdropColorScheme` + `adaptedToBackdrop`. Prior rev: PR #590 — row hairlines removed product-wide in favour of spacing-only separation (rows never get frames); `TimerScreenView` context-pill strip removed, `skinAccent` helper added; `WorldClockInlineLine` is now a snap carousel with edge fades, not truncated text; `Banner/ProactiveCapacityNotice.swift` deleted (63 components, was 64); `BacklogFullscreenView`/`BacklogScreenModel` per-day working-hours + edge-to-edge footer chrome. Prior rev: PR #584 R4 — backlog fullscreen's hard-state action rail deleted (`SmartActions`, `BacklogSmartActionsRow`), header «Plan N» pill is now the screen's only planner verb; R5 header/free-slot copy changes documented in `../concepts/menu-bar-popover.md`. PR #586 — `BacklogScreenModel.remainingWorkdayMinutes` drops `workingDays`, UX_AUDIT F10 resolved)
...
| `FullScreenAlert/FullScreenAlertView.swift` | `struct FullScreenAlertView` (`:4`) | 361 | Pre-meeting takeover. Live countdown, action row is **Dismiss-leading / Join-trailing** (PR #593, HIG: default action sits trailing), Join is a flat accent fill (gradient removed), optional next-event hint, snooze menu, skin-tinted overlay. Bell icon skips `.symbolEffect` entirely under Reduce Motion instead of just slowing it |
...
65 SwiftUI components across `Components/{Background,Backlog,Banner,Common,Event,Picker,Slot}/` (was 64 — `Common/SnippetRowChrome.swift` added, PR #607). Grouped by role; line numbers cite the main type declaration.
...
### Event/task rows (5)
- `EventRowView` (`:4`)
- `BacklogTaskRow` (`:10`) — two-deck `VStack`: `titleLine` + `metaLine`, the latter capped at `metaBudget = 4` meta items with a `+N` overflow tail (`BacklogTaskRow+Subviews.swift`)
- `GhostEventRow` (`:14`)
- `FreeSlotRow` (`:13`) — first-class row inserted between events; drag-drop fill-slot handling
- `ContextualActionRow` (`:16`) — single action row with icon, verb, optional subtext, async run/discover handling

Since PR #607, `EventRowView` and `BacklogTaskRow` share one row-chrome implementation instead of two hand-rolled variants: `Components/Common/SnippetRowChrome.swift` provides `RowStateStripe` (the leading state-stripe bar, used by both the backlog scheduled/urgent stripe and the event color-tag stripe) and the `snippetRowChrome(isHovered:isFocused:isDropTargeted:)` modifier (hover fill + keyboard-focus ring + drop-target highlight). `DS.Size.rowMinHeight` (`DesignSystem+Sizes.swift`, 40pt) is now the single min-height for event, backlog-task, free-slot, and ghost rows. `BacklogTaskRow.deadlineTint(_:calm:)` centralises the deadline→colour cascade (today/overdue → destructive, tomorrow → urgent, else the caller's calm colour) behind both the title and meta-text colours. `DS.dayTimeLabel(_:)` (`DesignSystem+Formatting.swift`) renders the compact «Today 15:14» / «Tue 14:00» scheduled-when label used by `BacklogTaskRow`'s scheduled meta item.
...
- `BacklogTombstones` (`:29`) — completed-today and frozen tasks with expand/collapse and restore
...
- `SyncStaleBannerRow` (`Banner/SyncStaleBannerRow.swift:13`) — interactive capsule (PermissionBannerRow voice) for a stalled sync pipeline; names the last successful refresh, click retries `ReminderService.syncNow()`
...
| `Components/Backlog/BacklogTaskRow.swift` | 643 | Single-row component, now a two-deck `content` (`BacklogTaskRow+Subviews.swift:146`): `titleLine` (`:159`) + `metaLine` (`:268`, `metaBudget = 4` items + `+N` tail). Row chrome (hover/focus/drop-target) and the leading state stripe come from the shared `Components/Common/SnippetRowChrome.swift`. `BacklogTaskRow+Subviews.swift` (603) holds drag payload + checkbox/content/controls/background/focus-ring/scheduled-chip helpers; `OverduePulseDot.swift` (25) lifted to its own file. |
| `Components/Event/EventRowView.swift` | 695 | Single-row component; hover fill, keyboard-focus ring, and the leading state stripe now come from the shared `SnippetRowChrome.swift` (`snippetRowChrome()`, `RowStateStripe`) instead of hand-rolled `Rectangle` overlays, matching `BacklogTaskRow`. Three clusters split out 2026-05-12: `+Title.swift` (109, inline rename gate + commit/cancel), `+DragReschedule.swift` (134, vertical drag with minute snapping + preview badge), `+HoverActions.swift` (211, snooze/complete/disintegration-delete/reminder menu). |
