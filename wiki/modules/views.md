...
| `Timer/TimerScreenView.swift` | `struct TimerScreenView` (`:4`) | 721 | Pomodoro/event timer. Live countdown ring. **Scrub gesture** adjusts `endDate`. **Pause gesture** shifts start+end. Compact header context pills (time range + location) reduce hunt time. Wallpaper support. Pinned state |
| `Backlog/BacklogFullscreenView.swift` | `struct BacklogFullscreenView` (`:38`) | 793 | Full-popover task list. Inline editing, drag-reorder, urgency filter, smart-sort, hotkeys 1–9 for completion, optimizer presets, schedule/deadline actions. Focus summary pills (active count, workload, today-left) above task stream. Three logic clusters in sibling files: `+BulkActions`, `+Reorder`, `+Actions`. |
...
| `Backlog/BacklogFullscreenView.swift` | 793 | Body + computed properties + subview helpers + filter chrome. Three logic clusters split out 2026-05-12: `+BulkActions.swift`, `+Reorder.swift`, `+Actions.swift`. All `@State`/`@FocusState`/`@Environment` wrappers are internal so siblings can read/write. |
