...
> **Last ingest:** 2026-06-11 (rev: `MenuBarScreenModel` introduced; `MenuBarView` down to 182 L / 9 `@State` fields / 10 extensions; deleted `+Timeline`, `+Strings`, `+Permissions`, `+Focus`; PR #567)
...
|---|---|---:|---|
| `MenuBar/MenuBarView.swift` | `struct MenuBarView` (`:4`) | 182 | Popover root — composition only. `var body` is ~50 L: background + `navigationDestination()` + command palette + toast + keyboard shortcuts, plus the modifier chain. Ten logic/view clusters live in sibling extension files: five original (`+AutoDefer`, `+RollForward`, `+Pomodoro`, `+BacklogDrop`, `+EventActions`) plus five from the 2026-05-12 body decomposition (`+NavigationRoutes`, `+MainContent`, `+Lifecycle`, `+EventRow`, `+DayGroup`). Timeline/filter/nav state and all pure derived computation moved to `MenuBarScreenModel` (`MenuBarScreenModel.swift:15`) stored as `@State var screen` (PR #567); `+Timeline`, `+Strings`, `+Permissions`, and `+Focus` were deleted. The standalone focus-summary pill row and the separate command bar were removed in PR #553 — stats surface through day-section header subtitles; the optimizer entry point is the eyebrow+title block itself (Button → command palette). Nested types extracted to file scope: `MenuBarNavigation`, `MenuBarPaletteContext`, `MenuBarDayListItem`, `MenuBarTimelineDay`. Permission banners + settings button live in `Presentation/Views/Components/`; preference keys (`OptimizerBottomKey`) in `Presentation/Views/MenuBar/MenuBarPreferenceKeys.swift:13` |
...
|---|---:|---|
| `MenuBar/MenuBarView.swift` | 182 | View-machinery `@State` only (9 fields) after PR #567 diet. Body = `AppBackgroundLayer` → `Group { navigationDestination() }` → `commandPaletteOverlay()` → `ToastOverlay` → `keyboardShortcutsLayer()`, plus modifier chain. Ten sibling extensions: **five original** — `+AutoDefer`, `+RollForward`, `+Pomodoro`, `+BacklogDrop`, `+EventActions`; **five from 2026-05-12 body decomposition** — `+NavigationRoutes`, `+MainContent`, `+Lifecycle`, `+EventRow`, `+DayGroup`. State surface: `screen: MenuBarScreenModel` (all timeline/filter/nav + derived computation), `backlogCoordinator: BacklogInteractionCoordinator`, `dayRolloverTimer`, `toastState`, `scrollPositionID`, `listScrollY`, `showingQuickCapture`, `dismissedBannerIds`, `optimizerBottomY`. `everyMinuteTimer` is now a `private let` (not `@State`) feeding `screen.nowTick`. |
