...
> **Last ingest:** 2026-06-10 (rev: PR #561 — removed focus summary pills + suggestion banner from BacklogFullscreenView; SmartActions `planChip` replaces `moreChip`/popover; CommandPalette «All presets» section grouped by category; BacklogHeader ETA chip removed, eyebrow now `SectionLabel` inline-only; `DS.Typography.label(skin:)` deleted)
...
| `Backlog/BacklogFullscreenView.swift` | `struct BacklogFullscreenView` (`:38`) | 672 | Full-popover task list. Inline editing, drag-reorder, urgency filter, smart-sort, hotkeys 1–9 for completion, optimizer presets, schedule/deadline actions. Three logic clusters in sibling files: `+BulkActions`, `+Reorder`, `+Actions`. The focus summary pills (`backlogFocusSummary`) and ready-to-plan suggestion banner were removed in PR #561. `BacklogSmartActionsRow` now mounts only when `capacityForecastIsHard` (`.over`/`.afterHours`); soft states rely on the header's persistent «Plan N» pill. |
| `CommandPalette/CommandPalette.swift` | `struct CommandPalette` (`:16`) | 796 | Smart suggestion palette. AI intent composition, event/task search, optimization presets, **dry-run preview**, "power mode" for advanced users. Three clusters in sibling files: `+PowerMode`, `+Status`, `+Actions`. Moved 2026-05-12 from `Views/Common/` to its own folder. The «All intents» disclosure section was renamed to **«All presets»** (PR #561) and now renders presets grouped by `IntentPresets.allCategories` (Planning / Focus / Deadlines / Meetings / Energy / Adjustments) with per-preset SF Symbol icons via `presetIcon(_:)`. |
| `DesignSystem/DesignSystem.swift` + 7 siblings | `enum DS` (`:21`), `enum Haptics` (`:30`) | 541 + 900 (sum of 7 siblings) | Catalog split across `DS+Layout` (Spacing, Density, Hero, Popover, Grid, SettingsWindow, EmptyState — 112 L), `DS+Typography` (160 L), `DS+Sizes` (Component sizes, Border, Opacity — 194 L), `DS+Visual` (Shadows, Elevation, Physics, Animation — 170 L), `DS+Colors` (Semantic, Materials, EventColorTag, Urgency, Countdown — 99 L), `DS+Formatting` (SnoozeOption, Ordinal, Time, Shared formatters — 71 L), `DS+Prototype` (137 L). `DesignSystem.swift` itself keeps the `enum DS` namespace + the `Haptics` enum + the View / Text / EnvironmentValues extensions that depend on DS tokens. Call sites unchanged (`DS.Spacing.sm`, `DS.Colors.surfacePrimary`, …). `DS.Typography.label(skin:)` (tracked-uppercase caption2) was deleted in PR #561 — the 2026 section-header voice is `SectionLabel` / `sectionHeaderStyle()` (mixed-case). `SectionLabel` gained an optional `tint: Color?` parameter (`DesignSystem.swift`) so hosts can supply an accent colour without forking the component. |
...
### Backlog (5)
- `BacklogHeader` (`:48`) — mode-aware (inline / fullscreen). Internal layout split by PR #553 into a **title block** (`titleBlock`: `SectionLabel("Backlog")` eyebrow (inline mode only — fullscreen suppresses it to avoid repeating the title) + capacity ring + count) and a **toolbar** (`toolbar`: smart-sort glyph, plan capsule, project picker, optional fullscreen button). Capacity verdict and urgent pill remain below the header row in both modes. The ETA chip (`BacklogETAChip`) was removed in PR #561; `BacklogHeader` retains the `EtaContent: View` generic via an `extension BacklogHeader where EtaContent == EmptyView` convenience init.
...
- `SmartActions` (`:25`) — chip row driven by capacity forecast. Hard / soft state contributes one primary verb; ranked calm actions (`rankedCalmActions`) surface in calm state. Trailing `planChip` (wand.and.stars «Plan», opens ⌘K palette) replaced the old `moreChip` / preset popover in PR #561 — the palette is now the single preset home. `showsPlanChip: Bool = true` lets the fullscreen backlog suppress the chip when its header already carries a persistent «Plan N» pill.
- `SmartActionsBar` (`:13`)
...
| `Backlog/BacklogFullscreenView.swift` | 672 | Body + computed properties + subview helpers + filter chrome. Three logic clusters split out 2026-05-12: `+BulkActions.swift`, `+Reorder.swift`, `+Actions.swift`. All `@State`/`@FocusState`/`@Environment` wrappers are internal so siblings can read/write. |
| `CommandPalette/CommandPalette.swift` | 796 | NL intent / quick action search. Three clusters split out 2026-05-12: `+PowerMode.swift`, `+Status.swift`, `+Actions.swift`. |
