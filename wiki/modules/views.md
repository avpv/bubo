...
| `Backlog/BacklogFullscreenView.swift` | `struct BacklogFullscreenView` (`:38`) | 686 | Full-popover task list. Inline editing, drag-reorder, urgency filter, smart-sort, hotkeys 1–9 for completion, optimizer presets, schedule/deadline actions. Row verbs injected once via `.environment(\.backlogRowActions, …)` (PRINCIPLES.md §9). Three logic clusters in sibling files: `+BulkActions`, `+Reorder`, `+Actions`. |
...
- `BacklogRowActions` (`BacklogRowActions.swift:21`, `struct`) — environment-injected action bundle for backlog rows. Non-optional fields are always-available verbs (complete, edit, delete, freeze, reorderDrop, move, focus navigation); optional fields gate their UI affordance on nil. Installed once by the host via `.environment(\.backlogRowActions, …)`; rows read `@Environment(\.backlogRowActions)` without any intermediate parameter wiring (PRINCIPLES.md §9).
...
| `Backlog/BacklogFullscreenView.swift` | 686 | Body + computed properties + subview helpers + filter chrome. Three logic clusters: `+BulkActions.swift`, `+Reorder.swift`, `+Actions.swift`. PR #565: `BacklogFullscreenTaskRow` deleted (was a 21-parameter forwarding shim); row verbs moved to `\.backlogRowActions` environment key (−79 lines net). |
...
| `Components/Backlog/BacklogTaskRow.swift` | 650 | Single-row component. Accepts data parameters only (task, flags, computed hints) plus three drag side-channels (`onDragStart`, `onDragEnd`, `onHoverChanged`); all verbs come from `\.backlogRowActions`. `BacklogTaskRow+Subviews.swift` (620) holds drag payload + checkbox/content/controls/background/focus-ring/scheduled-chip helpers; `OverduePulseDot.swift` (25) lifted to its own file. |
...
- Design rules live in `PRINCIPLES.md` (repo root, §1–§10) and are cited from code as `PRINCIPLES.md §N`. See [`../concepts/design-principles.md`](../concepts/design-principles.md).
