...
## [2026-07-18] ingest | Backlog/event rows unified on shared SnippetRowChrome

- **Trigger:** PR #607 (merge 2b56029fc71a)
- **Touched:** wiki/modules/views.md
- **Notes:** Backlog task rows restructured to a two-deck layout (title deck + a `metaBudget = 4` metadata deck with `+N` overflow); new `Components/Common/SnippetRowChrome.swift` (`RowStateStripe` + `snippetRowChrome()`) replaces hand-rolled hover/focus/drop-target overlays in both `BacklogTaskRow` and `EventRowView`; `DS.Size.rowMinHeight` (40pt) replaces the split `eventRowMinHeight`/`backlogRowHeight`/`BacklogTaskRow.compactRowHeight` constants. While in the size-hotspots table, also corrected several line counts (`BacklogTaskRow.swift`, `+Subviews.swift`, `EventRowView.swift`, `+Title.swift`, `+HoverActions.swift`) that had already drifted from the actual source before this PR — fixed since this diff forced a rewrite of the same table cells anyway.
