...
## [2026-05-13] ingest | PR #522 — Refactor: Extract large optimizer models and intent types into separate files

- **Trigger:** PR #522 (dd165ac5ec8e)
- **Touched:** wiki/modules/tests.md, wiki/architecture/layered-structure.md, wiki/modules/services.md, wiki/modules/app.md
- **Notes:** PR #522's own Step D commit updated the optimizer/fitness/intent pages but left four pages with stale path citations from the same PR's six mechanical moves. Corrected: `Tests/BuboTests/GACore/` → `GeneticAlgorithm/` in tests.md; `Infrastructure/System/` expanded to `Bundle/Cache/Network/Security/` and `Presentation/Coordinators/` split off `State/` in services.md and layered-structure.md; `Composition/App.swift` → `Composition/App/App.swift` in app.md.
