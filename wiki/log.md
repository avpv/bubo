...
## [2026-07-25] ingest | Deleted events kept firing full-screen alerts

- **Trigger:** branch claude/event-deletion-alert-bug-tovuvz (owner report: «событие удалилось но почему-то по нему все равно показывает полноэкранный алерт»)
- **Touched:** wiki/concepts/full-screen-alerts.md, wiki/concepts/notifications-bus.md
- **Notes:** `NotificationScheduler.schedule(_:)` only ever touched the events it was handed, so an event that stopped arriving in the sync slice (deleted in Calendar.app, or filtered out by a calendar-selection change) kept its `Timer` and still fired. Three layers now: (1) `reconcile(liveEventIds:)` cancels timers for every id missing from the *complete* live set, called from `ReminderService.onEventsUpdated` with the external slice plus every expanded local occurrence (`liveEventIds(external:)` — local events never arrive through `EventKitSyncCoordinator`, so omitting them would cancel their reminders every sync tick); (2) fire-time guards in `fire` and `firePhaseAlert` for the delete that lands between reconciles; (3) new `.eventsDidDisappear` notification → `AppDelegate.dismissAlerts(forEventIds:)`, which drops queued alerts, tears down the alert on screen, dismisses the Join ribbon (whose «Re-alert» would otherwise resurrect it), and nils a vanished back-to-back `nextEvent`. Also replaced `lastScheduledEvents` with a merged `knownEvents` dictionary — the old assignment let a partial `schedule([one])` call shrink the J4 back-to-back lookup's world to a single event. `pruneFiredReminders` now receives the full live set instead of external-only. Tests: reconcile/back-to-back cases in NotificationSchedulerTests, disappearance-announcement cases (local delete, occurrence exclusion, Apple-Calendar delete, local-events-survive-sync) in ReminderServiceTests.
