...
## [2026-07-18] ingest | PR #599 — Core audit fixes: recurrence, constraint checking, and state management

- **Trigger:** PR #599 (4c93895183ef)
- **Touched:** wiki/concepts/pomodoro.md, wiki/modules/tests.md
- **Notes:** PR #599's own commits (branch `claude/solution-audit-critique-swkauo`) already self-logged the three ingest entries above covering the bulk of the merge (shared capture commit path, core-audit correctness fixes, working-hours clamp). This pass audited the full merge diff (78 files, +1803/−514) against current wiki claims for gaps: found one — `PomodoroConfigResolver.resolveShape` now reserves the long break *inside* the total-minutes budget before packing rounds (previously bolted on top and stripped back off by the fit pass, so long breaks were dead for any tightly packed target); added to `pomodoro.md`'s Rhythm resolution bullet. Also caught that `Tests/Domain/RecurrenceExpanderTests.swift` and `Tests/Optimizer/GeneticAlgorithm/CPSATSolverCorrectnessTests.swift` (both new in this PR) were never reflected in `tests.md`'s file counts/listings — updated 67→69 total, Domain 3→4, `Optimizer/GeneticAlgorithm/` 14→15, added a Recurrence category. `NotificationScheduler.swift`'s new `notificationCenterAvailable` gate (guards `UNUserNotificationCenter` calls under `swift test`, which lacks an LaunchServices `.app` host) was checked against `notifications-bus.md`/`full-screen-alerts.md` — no page documents `firePhaseAlert`/`showSystemNotification`/`requestNotificationPermission` at this level of detail, and the gate is test-environment-only (production `.app` bundles are unaffected), so left as log-only, consistent with the working-hours-clamp precedent above. Pre-existing, out-of-scope drift noted for a future lint pass: `tests.md`'s `App/` subtree claims 22 files, actual count is 21 — not touched by this PR's diff.
