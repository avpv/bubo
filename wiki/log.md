...
## [2026-07-18] ingest | PR #603 — Fix day-nav focus tracking and improve header control affordances

- **Trigger:** PR #603 (f75e32b5a035)
- **Touched:** none
- **Notes:** PR #603's own commits (branch `claude/schedule-scroll-today-92jvg1`) already self-logged the three ingest entries above covering the full merge (day-nav focus/Today-always-live, `HeaderControlStyle` hover-pills, filter count badge). Audited the diff (`EventList.swift`, `DesignSystem.swift`, `MenuBarScreenModel.swift`, `MenuBarView+MainContent.swift`) against `menu-bar-popover.md`'s "Header trailing cluster" section and header block: `onVisibleDayChange`/`DayHeaderMinYKey` mechanics, `focusedDayIsToday` removal, the `eventListTop` fallback scroll, `HeaderControlStyle`'s hover/press/disabled behaviour, and `activeTimelineFilterCount` badge all match current source exactly. Checked other pages citing the touched files' basenames (`skins-system.md`, `modules/skins.md`, `modules/views.md`, `architecture/layered-structure.md`, `concepts/quick-capture.md`): all cite `DesignSystem.swift`/`MenuBarView+MainContent.swift` only for unrelated facts (the `DS` namespace, `resolvedHoverFill`'s pre-existing definition, `handleQuickAddTask`) that this diff didn't touch. No corrections needed, no page flagged for retirement, no new page warranted (`HeaderControlStyle` is already documented inline in `menu-bar-popover.md`, consistent with how `IconPressStyle` was handled).
