...
## [2026-07-18] ingest | PR #601 — Make working-hours boundary handles interactive on every day

- **Trigger:** PR #601 (3a82846b17b0)
- **Touched:** none
- **Notes:** PR #601's own commits (branch `claude/work-day-duration-config-shslt9`) already self-logged the two entries directly above, covering the full merge (reveal-button variant, then its supersession by always-interactive rows). Audited `menu-bar-popover.md`'s working-hours paragraph and header against current `WorkingHoursBoundaryRow.swift` (the `isInteractive` gate, its step-chevron/gesture/accessibility conditionals, are fully removed) and `MenuBarView+DayGroup.swift` (`dayIsToday` no longer gates row presence; now-anchored hiding is today-only, future days always render both) — claims match. `WorkingHoursBoundaryRow.swift` isn't cited by filename in any page's `Sources:` header but falls under the already-listed `Bubo/Presentation/Views/Components/` root, so no header change was needed. The PR's third file, `Tests/Optimizer/GeneticAlgorithm/GATests.swift`, only de-flakes two test assertions (a shared-Date fix and a sampling-noise tolerance) — no behavior change, not cited by any page, no wiki action.
