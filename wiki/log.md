...
## [2026-07-05] ingest | PR #580 — Consolidate MenuBarView session state into MenuBarScreenModel

- **Trigger:** PR #580 (611f1d3fe0a8)
- **Touched:** wiki/modules/views.md, wiki/concepts/menu-bar-popover.md, wiki/modules/tests.md
- **Notes:** Biggest correction: `MenuBarView` row and Sources headers still listed `+AutoDefer`/`+EventActions`/`+Timeline`/`+Strings`/`+Permissions`/`+Focus` as live sibling extensions — all six are gone (the last two merged into `+Lifecycle`/`+EventRow` in this PR, the first three folded into `MenuBarScreenModel` in an earlier, unlogged PR #567); MenuBarView is now 2 `@State` fields + 8 sibling extensions, with `MenuBarScreenModel` documented for the first time as the popover's session-state owner. Documented the new Unscheduled shelf (REDESIGN.md R1), the strip-free resting screen with the world clock folded into an inline header line (REDESIGN.md R2), and the unified Quick Add front door (UX_AUDIT.md F8, `QuickAddParser`/`QuickAddView`) — added to views.md's Backlog/Utility/Quick-Add component tables and menu-bar-popover.md. Added `QuickAddParserTests` to tests.md (67 → 68 files). Skipped: PRs #567–#570 (same-day merges as #566) never received a wiki-ingest log entry and left `views.md`/`menu-bar-popover.md` stale beyond this PR's diff (e.g. `MenuBarScreenModel` itself predates #580 but was undocumented) — flagging for a human-requested lint/resync pass rather than backfilling here.
