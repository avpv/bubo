...
## [2026-07-18] ingest | PR #604 — Add sync staleness watchdog and menu-bar failure indicator

- **Trigger:** PR #604 (0b7a7b5f342e)
- **Touched:** wiki/modules/tests.md
- **Notes:** PR #604's own branch commits already self-logged the watchdog/mark/status-branch changes across `event-pipeline.md`, `menu-bar-popover.md`, and `views.md` (see the ingest entry directly above) and those claims check out against current source. This pass's only gap: the new `Tests/App/Presentation/SyncHealthEvaluatorTests.swift` wasn't reflected in `tests.md`'s file-count/layout/coverage tables — fixed, and the file count corrected 67 → 70 (2 of those were already-undocumented files pre-dating this PR; flagged for a future full resync rather than chased down here).
