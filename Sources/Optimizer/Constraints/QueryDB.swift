...
public struct QueryKey: Hashable, Sendable {
    ...
    public init(_ domain: String, _ identifier: String) {
        self.domain = domain
...
/// to drop.
public final class QueryTracker: Sendable {
    ...
    public func read(_ input: QueryKey) {
...
public final class QueryDB<Output: Sendable>: Sendable {
    ...
    /// "this changed" signal.
    public func setInput(_ key: QueryKey) {
    ...
    /// fingerprint at once.
    public func setInputs<S: Sequence>(_ keys: S) where S.Element == QueryKey {
        inputs.withLock { state in
            for key in keys {
                let prev = state[key]?.revision ?? 0
                state[key] = InputState(revision: prev &+ 1)
    ...
    public func query(_ name: QueryKey, _ build: (QueryTracker) -> Output) -> Output {
    ...
    public func invalidateQuery(_ name: QueryKey) {
