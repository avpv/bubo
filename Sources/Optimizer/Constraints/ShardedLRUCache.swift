...
public final class ShardedLRUCache<Value: Sendable>: Sendable {
    ...
    public init(capacity: Int, shards shardCount: Int = 8) {
    ...
    public func value(forKey key: UInt64, build: () -> Value) -> Value {
    ...
    public func lookup(forKey key: UInt64) -> Value? {
        let shardIndex = Int(key % UInt64(shardCount))
    ...
    /// LRU touch.
    public func store(forKey key: UInt64, value: Value) {
        let shardIndex = Int(key % UInt64(shardCount))
