...
public struct ReferencePoints {
    ...
    /// from Deb & Jain, 2014.
    public static func dasDennis(dimension m: Int, divisions p: Int) -> ReferencePoints {
    ...
    public static func auto(dimension m: Int, targetCount: Int) -> ReferencePoints {
        precondition(m > 0, "objective count must be positive")
        if m == 1 {
...
public struct NSGA3 {
    ...
    public init(referencePoints: ReferencePoints) {
    ...
    public static func forPopulation(objectiveCount m: Int, populationSize n: Int) -> NSGA3 {
    ...
    public func select(_ vectors: [[Double]], count k: Int) -> SelectionResult {
        precondition(k >= 0)
        let n = vectors.count
        guard n > 0 else {
    ...
    public func rankAll(_ vectors: [[Double]]) -> SelectionResult {
        let fronts = nonDominatedSort(vectors)
        var frontOf: [Int: Int] = [:]
        for (rank, front) in fronts.enumerated() {
            for idx in front { frontOf[idx] = rank }
    ...
    public static func applyScalarFitness<C: Chromosome>(
    ...
    public func nonDominatedSort(_ vectors: [[Double]]) -> [[Int]] {
        let n = vectors.count
    ...
    public func normalize(vectors: [[Double]]) -> [[Double]] {
    ...
    public func associate(vector v: [Double]) -> (Int, Double) {
    ...
    public func perpDistance(_ v: [Double], to w: [Double]) -> Double {
