...
public struct SlotRegistry: Sendable {
    ...
    public static let defaultStride: TimeInterval = 15 * 60
    ...
    public func stridesForMinutes(_ minutes: Int) -> Int {
    ...
    public func resolvedDate(at index: Int) -> Date? {
    ...
    public static func build(from context: OptimizerContext) -> SlotRegistry {
    ...
    /// up to 5.
    public static func autoDetectStride(
...
public final class SlotRegistryHolder: @unchecked Sendable {
    private var cached: SlotRegistry?
    private let lock = NSLock()

    public init(preloaded: SlotRegistry? = nil) {
    ...
    public func get(for context: OptimizerContext) -> SlotRegistry {
        lock.lock()
        if let cached {
            lock.unlock()
            return cached
    ...
    public func preload(_ registry: SlotRegistry) {
        lock.lock()
        cached = registry
        lock.unlock()
    }
