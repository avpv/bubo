...
public final class TabuMemory: @unchecked Sendable {
    ...
    public func markMoves(eventIds: [String]) {
    ...
    /// a tabu.
    public func noteBest(hardTier: Double, softTier: Double) {
        lock.lock()
        defer { lock.unlock() }
    ...
    public func isTabu(eventId: String) -> Bool {
        lock.lock()
        defer { lock.unlock() }
        let cutoff = currentStep - config.tenure
    ...
    public func longTermFrequency(eventId: String) -> Int {
        lock.lock()
        defer { lock.unlock() }
    ...
    public func score(eventId: String) -> Double {
        lock.lock()
        defer { lock.unlock() }
        let cutoff = currentStep - config.tenure
    ...
    public func aspires(hardTier: Double, softTier: Double) -> Bool {
        lock.lock()
        defer { lock.unlock() }
    ...
    public var activeTabuCount: Int {
        lock.lock(); defer { lock.unlock() }
        let cutoff = currentStep - config.tenure
