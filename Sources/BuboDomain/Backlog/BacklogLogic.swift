...
public enum BacklogLogic {
    ...
    public static func activeTasks(_ tasks: [BacklogTask]) -> [BacklogTask] {
    ...
    public static func urgentTasks(
        _ tasks: [BacklogTask],
    ...
    public static func matchesSmartFilter(
        _ task: BacklogTask,
        filter: SmartFilter,
        now: Date = Date(),
        calendar: Calendar = .current
    ...
    public static func smartFilterCounts(
        _ tasks: [BacklogTask],
        now: Date = Date(),
        calendar: Calendar = .current
    ) -> [SmartFilter: Int] {
    ...
    public static func smartSorted(
        _ tasks: [BacklogTask],
        now: Date = Date(),
        calendar: Calendar = .current
    ) -> [BacklogTask] {
    ...
    public static func completedToday(
        _ tasks: [BacklogTask],
        now: Date = Date(),
        calendar: Calendar = .current
    ) -> [BacklogTask] {
    ...
    public static func capacityForecast(
        pendingMinutes: Int,
    ...
    /// whatever it gets.
    public static func capacityPartition(
        _ tasks: [BacklogTask],
        remainingWorkdayMinutes: Int
    ...
    // `proposedSlotsFromShadow(_:)` lives in
    // `Application/Backlog/BacklogLogic+ShadowSlots.swift` because its
    // parameter type (`ScheduleScenario`) is an Optimizer type and
    // Domain must not depend on Optimizer.
    ...
    public static func naiveProposedSlots(
    ...
    public static func remainingWorkdayMinutes(
        workingHours: ClosedRange<Int>,
        now: Date = Date(),
        calendar: Calendar = .current,
        workingDays: Set<Int> = []
