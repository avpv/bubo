...
| R2 | **Declutter the resting screen.** World-clock band → one quiet inline line in the header block; colour-filter band → behind a filter glyph in the day-nav cluster (band visible only while filtering). ⚠️ Supersedes the owner's earlier «keep the strips» choice under the newer «полностью переделывать» mandate — **re-confirm before building**. | proposed |
