...
| R3 | **One Plan verb.** The main screen's chip rail (hard «Schedule overflow» / soft suggestion / ranked calm action / Plan / Backlog-entry chip) collapses into a single adaptive «Plan» chip — label carries the forecast («Plan», «Plan · 2 h over» warning-tinted, «Plan · 2 h queued» quiet), tap opens the palette. `SmartActionsBar` deleted; the Backlog chip died with it (it duplicated the Unscheduled shelf — §3, confirmed on the 2026-07-05 build screenshots); ⇧⌘N now opens the unified Quick Add on the footer. Conscious trades: one-tap hard-state runs become two taps (palette is the single home); the transient post-run «Done · why?» reasoning chip left the resting screen (the undo toast remains) — it can resurface inside the palette later. | **landed 2026-07-05** |
| R4 | **Backlog fullscreen → management only.** The hard-state SmartActions row is gone — it duplicated the header's «Plan N» pill (§1), which is now the screen's single planner verb; the capacity verdict under the count keeps carrying the diagnosis. `SmartActions` + `BacklogSmartActionsRow` deleted (last mounts died here; the reasoning/soft-suggestion machinery they carried lives on in the palette's presets), `capacityForecastIsHard` gate removed, three optimizer pipes (`onFocusOnDeadlines`, `onRunRequest`, `onOpenPalette`) unwired from the screen. Selection, bulk ops, freeze/tombstones, projects untouched. | **landed 2026-07-05** |
| R5 | **Now-anchored header + polish.** Landed: the header subtitle leads with execution when something is live — «Now: Standup · until 14:00 · next in 2 h» — falling back to the count verdict; today's day-section summary is empty (its count/next/working-hours all lived in the popover header one band above — the «No events today» / «9–22 · 0 events» double from the build screenshots); working-hours boundary rows render on today only (one global rule, no longer repeated down every future day); free-slot durations ≥ 2 h round to «~11½ h» (exact range stays right below). Remaining from the original R5 scope: **F9 form ergonomics** (duration pills in the event form, «Add to Calendar» caption) — small, independent, still open. | **landed 2026-07-05** (F9 open) |
