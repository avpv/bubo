...
struct SlotDomain: Sendable {
    ...
    /// is active.
    func indices(after floor: Date, registry: SlotRegistry) -> ArraySlice<Int> {
    ...
    static let empty = SlotDomain(indices: [])
...
final class SlotDomainsHolder: @unchecked Sendable {
    ...
    func domain(for eventId: String, in context: OptimizerContext) -> SlotDomain {
