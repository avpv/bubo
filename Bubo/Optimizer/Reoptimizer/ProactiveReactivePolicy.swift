...
enum ScheduleDisturbance: Sendable {
...
struct ScheduleRecovery: Sendable {
    /// Existing genes to reschedule (eventId → new startTime).
    let shifts: [String: Date]
    ...
    let reflowAfter: Bool

    static let noop = ScheduleRecovery(
        shifts: [:], removals: [], insertions: [], reflowAfter: false
    )
}

final class ProactiveReactivePolicy: @unchecked Sendable {
