...
struct ScenarioPairCandidate: Sendable {
    let id: String
    let aScores: [String: Double]
    let bScores: [String: Double]
}

/// Ranking score for one candidate pair.
struct ActiveLearningRank: Sendable {
    let id: String
    let probability: Double   // p(A ≻ B) under current weights
    let entropy: Double       // base entropy
    let magnitude: Double     // L2 norm of score-delta
    let value: Double         // entropy × magnitude — total information value
}

enum ActiveLearningSampler {
...
enum ScenarioPairActiveSelector {
