...
struct SlotRegistry: Sendable {
    ...
    /// Grid stride in seconds. Fixed at 15 minutes to line up with
    /// every other snap-to-grid site in the optimizer
    /// (`findFirstFreeSlot`, mutation case 0/2, random sampler).
    static let stride: TimeInterval = 15 * 60
    ...
    static func build(from context: OptimizerContext) -> SlotRegistry {
        ...
        // Estimate capacity: days × (hours × 4 slots). Overshoots
        // slightly because some slots are filtered out; harmless.
        let hoursPerDay = context.workingHours.upperBound - context.workingHours.lowerBound
        var slots: [Date] = []
        slots.reserveCapacity(max(16, daysInHorizon * hoursPerDay * 4))
        ...
        return SlotRegistry(slots: slots, calendar: cal)
    }
}
...
extension SlotRegistry {
    ...
    static let empty = SlotRegistry(slots: [], calendar: .current)
}
