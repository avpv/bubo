...
struct PreferencePairEvent: Codable, Sendable, Hashable {
    let timestamp: Date
...
struct DurationSampleEvent: Codable, Sendable, Hashable {
    let timestamp: Date
    let title: String
    let context: String?
...
struct BranchingDecisionEvent: Codable, Sendable, Hashable {
    let timestamp: Date
    let regimeKey: Int
    let policy: String
    let reward: Double
...
struct EmbeddingContrastEvent: Codable, Sendable, Hashable {
    let timestamp: Date
    let anchor: [ScheduleGene]
    let target: [ScheduleGene]
    /// +1 = pull closer, -1 = push apart, magnitudes scale learning rate.
    let signedWeight: Double
}
...
enum TrainingEvent: Codable, Sendable, Hashable {
...
final class TrainingReplayBuffer: @unchecked Sendable {
    ...
    func append(batch: [TrainingEvent]) {
        lock.lock()
    ...
    func preferencePairs() -> [PreferencePairEvent] {
        lock.lock(); defer { lock.unlock() }
    ...
    func durationSamples() -> [DurationSampleEvent] {
        lock.lock(); defer { lock.unlock() }
        return events.compactMap {
    ...
    func branchingDecisions() -> [BranchingDecisionEvent] {
        lock.lock(); defer { lock.unlock() }
        return events.compactMap {
    ...
    func embeddingContrasts() -> [EmbeddingContrastEvent] {
        lock.lock(); defer { lock.unlock() }
        return events.compactMap {
    ...
    func events(from start: Date, to end: Date) -> [TrainingEvent] {
        lock.lock(); defer { lock.unlock() }
