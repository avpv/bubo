...
//
// Surface missing EventKit grants (Calendar / Reminders) as a tappable
// pill that deep-links into the matching Settings pane. One missing
// grant = a single pill; two = a paged horizontal carousel with a quiet
// dot indicator so the second banner doesn't stack and push the day
// list further down. The carousel sits ABOVE the timeline because
// missing permissions are blockers — they explain why the rest of the
// popover is empty.
...
/// The capsule pill itself — single tap opens the matching Settings
/// pane via the global `pendingPane` deep-link channel.
struct PermissionBannerLabel: View {
...
/// dot indicator beneath so the secondary banner shares a single
/// vertical slot instead of stacking.
